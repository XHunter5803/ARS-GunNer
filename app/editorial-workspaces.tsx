...
function ArticleWorkspace({ notify, autoDraft, onAutoDraftConsumed }: { notify: (message: string) => void; autoDraft?: AutoDraftRequest | null; onAutoDraftConsumed?: () => void }) {
  const [language] = useState<"th" | "en" | "bilingual">("th");
  ...
  if (!evidence) {
    return (
      <section className="grid min-h-[70vh] place-items-center rounded-[22px] border border-[#ded8e6] bg-white p-8 text-center shadow-sm">
  ...
  return (
    <section className="grid gap-4 2xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.55fr)]">
      <article className="rounded-[22px] border border-[#e3dee8] bg-white p-5 shadow-[0_12px_35px_rgba(31,20,45,.06)] sm:p-7">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[140px_1fr]">
          <div><label htmlFor="article-category" className="text-[10px] font-bold text-[#606775]">Category</label><input id="article-category" value={category} onChange={(event) => setCategory(event.target.value)} className="mt-2 h-10 w-full rounded-xl border border-[#ded8e3] px-3 text-xs" /></div>
          <div><label htmlFor="article-hashtags" className="text-[10px] font-bold text-[#606775]">Hashtags</label><input id="article-hashtags" value={hashtags} onChange={(event) => setHashtags(event.target.value)} className="mt-2 h-10 w-full rounded-xl border border-[#ded8e3] px-3 text-xs" /></div>
        </div>
        <label htmlFor="article-headline" className="mt-5 block text-[10px] font-bold text-[#606775]">พาดหัวเหตุและผล</label><textarea id="article-headline" value={headline} onChange={(event) => setHeadline(event.target.value)} rows={2} className="mt-2 w-full resize-none rounded-xl border border-[#ded8e3] bg-[#fffefd] p-3 text-sm font-bold leading-6 outline-none focus:border-[#53a99d]" />
        <div className="mt-4 space-y-3">{paragraphs.map((paragraph, index) => <div key={index}><div className="mb-1.5 flex items-center justify-between"><label htmlFor={`paragraph-${index}`} className="text-[10px] font-bold text-[#606775]">ย่อหน้าที่ {index + 1}</label><span className="text-[9px] text-[#9ba0a9]">{paragraph.length} ตัวอักษร</span></div><textarea id={`paragraph-${index}`} value={paragraph} onChange={(event) => updateParagraph(index, event.target.value)} rows={3} className="w-full resize-y rounded-xl border border-[#e1dce5] bg-[#fffefd] p-3 text-xs leading-6 outline-none focus:border-[#53a99d]" /></div>)}</div>
        <label htmlFor="closing-question" className="mt-4 block text-[10px] font-bold text-[#606775]">คำถามปิดท้าย</label><textarea id="closing-question" value={closingQuestion} onChange={(event) => setClosingQuestion(event.target.value)} rows={2} className="mt-2 w-full resize-y rounded-xl border border-[#ded8e3] p-3 text-xs" />
        <label htmlFor="article-signature" className="mt-4 block text-[10px] font-bold text-[#606775]">Signature</label><input id="article-signature" value={signature} onChange={(event) => setSignature(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#ded8e3] px-3 text-xs" />
        <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => void validate()} disabled={working} className="inline-flex items-center gap-2 rounded-xl bg-[#241432] px-4 py-3 text-[11px] font-bold text-white disabled:opacity-40"><ShieldCheck className="size-4 text-[#65ddcb]" />ตรวจบทความ</button><button type="button" onClick={() => void saveRevision()} disabled={working} className="inline-flex items-center gap-2 rounded-xl border border-[#dcd6e2] bg-white px-4 py-3 text-[11px] font-bold text-[#4e5664] disabled:opacity-40"><Save className="size-4" />บันทึก Revision</button><button type="button" onClick={() => setShowJson(!showJson)} className="inline-flex items-center gap-2 rounded-xl border border-[#dcd6e2] bg-white px-4 py-3 text-[11px] font-bold text-[#4e5664]"><FileJson2 className="size-4" />{showJson ? "ปิด Preview" : "Preview"}</button></div>
        {showJson ? <pre className="mt-4 max-h-[480px] overflow-auto rounded-xl bg-[#171021] p-4 text-[9px] leading-4 text-[#cbbfd5]">{JSON.stringify(article, null, 2)}</pre> : null}
      </article>
      ...
      <aside className="space-y-4">
        ...
        <article className="rounded-[22px] border border-[#e3dee8] bg-white p-5 shadow-sm">
          <p className="text-[9px] font-bold uppercase tracking-[.14em] text-[#8b919a]">Validation Chain</p>
          <h3 className="mt-1 text-xs font-extrabold">ใครตรวจข้อความ</h3>
          <div className="mt-3 space-y-2">
            {[
              { name: "News Discovery Agent", detail: "เลือกข่าวเกี่ยวข้องตามความหมาย", passed: researchSources.length > 0 },
              { name: "Fact-Checking Agent", detail: "ตรวจ Source ID, Facts, Claims และ Conflicts", passed: Boolean(research?.main_points.length) },
              { name: "Article Pattern Agent", detail: "ตรวจพาดหัว ย่อหน้า 5–7 ย่อหน้า และคำถามปิดท้าย", passed: Boolean(headline.trim() && paragraphs.length >= 5 && paragraphs.length <= 7 && paragraphs.every((item) => item.trim()) && closingQuestion.trim()) },
              { name: "Language Cleaner Agent", detail: "ตรวจภาษาและข้อความหลุดจากเว็บไซต์", passed: validation ? !validation.errors.some((item) => /ภาษา|ข้อความหลุด/.test(item)) : false },
              { name: "QA & Security Gate", detail: "คำนวณ Readiness และบล็อกการอนุมัติหากต่ำกว่า 85", passed: (validation?.readiness_score ?? 0) >= 85 },
              { name: "Human Editor", detail: "คุณตรวจและกดอนุมัติขั้นสุดท้าย", passed: workflowStatus === "approved" },
            ].map((check) => (
              <div key={check.name} className="flex items-start gap-2 rounded-xl border border-[#e8e3eb] p-2.5">
                {check.passed ? <CheckCircle2 className="mt-0.5 size-3.5 shrink-0 text-[#2f8f72]" /> : <Clock3 className="mt-0.5 size-3.5 shrink-0 text-[#b57837]" />}
                <div><p className="text-[9px] font-extrabold text-[#4f5662]">{check.name}</p><p className="mt-0.5 text-[8px] leading-4 text-[#8a9099]">{check.detail}</p></div>
              </div>
            ))}
          </div>
        </article>
        <article className="rounded-[22px] border border-[#e3dee8] bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h3 className="text-xs font-extrabold">Approval</h3><span className="text-[9px] font-bold text-[#8b919a]">{articleId ? `#${articleId}` : "Not saved"}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => void transition("review")} disabled={working || workflowStatus !== "draft"} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-[#edf2fb] px-3 py-2.5 text-[9px] font-bold text-[#3e67b5] disabled:opacity-40"><Send className="size-3.5" />ส่งตรวจ</button><button type="button" onClick={() => void transition("approved")} disabled={working || workflowStatus !== "review" || (validation?.readiness_score ?? 0) < 85} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-[#e8f8f2] px-3 py-2.5 text-[9px] font-bold text-[#26795f] disabled:opacity-40"><ThumbsUp className="size-3.5" />อนุมัติ</button></div><p className="mt-3 text-[9px] leading-4 text-[#9297a0]">อนุมัติได้เมื่อ Readiness ≥ 85 เท่านั้น</p></article>
        <article className="rounded-[22px] border border-[#e3dee8] bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><Languages className="size-4 text-[#466fc0]" /><h3 className="text-xs font-extrabold">ภาษา: {language}</h3></div></article>
      </aside>
    </section>
  );
}
